...
while read -u3 the_host ; do
   ...
   tmpvar=$(/usr/local/bin/ssh $the_host chuser login='true' rlogin='true' ebadmin )
   print "$the_host: $tmpvar chuser completed" >> $script_name.out  # Output file
   tmpvar=$(/usr/local/bin/ssh $the_host pwdadm -f ADMCHG ebadmin )
   if (( $? != 0 )); then
	print "Cannot connect to $the_host" >> $script_name.err
	continue	
   fi

   print "$the_host: $tmpvar pwdadm completed" >> $script_name.out  # Output file
